...
int SK_TR369_API_GetParams(const char *name, char *value, int size) {
    return (pFunc == nullptr) ? 0 : pFunc->SK_TR369_Callback_Get(OpenTR369CommandGet, value, size, name, nullptr);
}

int SK_TR369_API_SetParams(const char *name, const char *value) {
    return (pFunc == nullptr) ? 0 : pFunc->SK_TR369_Callback_Set(OpenTR369CommandSet, name, value, nullptr);
}

int SK_TR369_API_SendEvent(const char *name, const char *param_name, const char *param_value) {
    return (pFunc == nullptr) ? 0 : pFunc->SK_TR369_Callback_Set(OpenTR369CommandSet, "skyworth.tr369.event", param_name, param_value);
}

int SK_TR369_API_GetDatabaseStr(const char *name, const char *param, char *value, int size) {
    return (pFunc == nullptr) ? 0 : pFunc->SK_TR369_Callback_Get(OpenTR369CommandGetDatabaseStr, value, size, name, param);
}

int SK_TR369_API_SetProperty(const char *name, const char *value) {
    return (pFunc == nullptr) ? 0 : pFunc->SK_TR369_Callback_Set(OpenTR369CommandSetProperty, name, value, nullptr);
}
